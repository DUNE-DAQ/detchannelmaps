...
#ifndef DETCHANNELMAPS_PDSPCHANNELMAPSERVICE_HPP_
#define DETCHANNELMAPS_PDSPCHANNELMAPSERVICE_HPP_
...
public:
  ...
  // FEMB channel number is really the stream index number, but for FELIX, it is restricted to be in the range 0:127, so
  // really it's more like an FEMB channel number

  unsigned int GetOfflineNumberFromDetectorElements(unsigned int crate,
                                                    unsigned int slot,
                                                    unsigned int fiber,
...
private:
  ...
  unsigned int
    fvInstalledAPA[6]; // APA as installed.  This array maps the two conventions.  Argument = offline, value = installed
...
#endif // DETCHANNELMAPS_PDSPCHANNELMAPSERVICE_HPP_
