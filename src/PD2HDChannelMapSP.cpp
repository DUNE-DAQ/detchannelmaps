...
void
dune::PD2HDChannelMapSP::ReadMapFromFile(std::string& fullname)
{
  ...
  while (std::getline(inFile, line)) {
    ...
    linestream >> chanInfo.offlchan >> chanInfo.crate >> chanInfo.APAName >> chanInfo.wib >> chanInfo.link >>
      chanInfo.femb_on_link >> chanInfo.cebchan >> chanInfo.plane >> chanInfo.chan_in_plane >> chanInfo.femb >>
      chanInfo.asic >> chanInfo.asicchan >> chanInfo.wibframechan;
...
dune::PD2HDChannelMapSP::GetChanInfoFromWIBElements(unsigned int crate,
                                                    unsigned int slot,
                                                    unsigned int link,
...
dune::PD2HDChannelMapSP::HDChanInfo_t
dune::PD2HDChannelMapSP::GetChanInfoFromOfflChan(unsigned int offlineChannel) const
{
