...
void
dune::TPCChannelMapSP::ReadMapFromFile(std::string& fullname)
{
  fNChans = 0;
  ...
  while (std::getline(inFile, line)) {
    ...
    linestream >> chanInfo.offlchan >> chanInfo.detid >> chanInfo.detelement >> chanInfo.crate >> chanInfo.slot >>
      chanInfo.stream >> chanInfo.streamchan >> chanInfo.plane >> chanInfo.chan_in_plane >> chanInfo.femb >>
      chanInfo.asic >> chanInfo.asicchan;
    ...
    if (DetToChanInfo.find(hashval) != DetToChanInfo.end()) {
      std::cout << "TPCChannelMapSP: duplicate electronics channel found.  detid, crate, slot, stream, streamchan: "
                << chanInfo.detid << " " << chanInfo.crate << " " << chanInfo.slot << " " << chanInfo.stream << " "
    ...
    if (OfflToChanInfo.find(chanInfo.offlchan) != OfflToChanInfo.end()) {
      std::cout << "TPCChannelMapSP: duplicate offline channel found: " << chanInfo.offlchan << std::endl;
...
dune::TPCChannelMapSP::GetChanInfoFromElectronicsIDs(unsigned int detid,
                                                     unsigned int crate,
                                                     unsigned int slot,
                                                     unsigned int stream,
...
dune::TPCChannelMapSP::TPCChanInfo_t
dune::TPCChannelMapSP::GetChanInfoFromOfflChan(unsigned int offlineChannel) const
{
...
size_t
dune::TPCChannelMapSP::make_hash(unsigned int detid,   // 6 bits
