...
VDColdboxChannelMapService::VDColdboxChannelMapService(std::string filename)
{
  ...
  while (std::getline(inFile, line)) {
    ...
    linestream >> chinfo.offlchan >> chinfo.wib >> chinfo.wibconnector >> chinfo.cebchan >> chinfo.femb >>
      chinfo.asic >> chinfo.asicchan >> chinfo.connector >> chinfo.stripid;
    chinfo.valid = true;
...
VDColdboxChannelMapService::getChanInfoFromOfflChan(int offlchan)
{
  VDCBChanInfo r;
...
int
VDColdboxChannelMapService::getOfflChanFromWIBConnectorInfo(int wib, int wibconnector, int cechan)
{
  // int r = -1;
...
int
VDColdboxChannelMapService::getOfflChanFromSlotFiberChan(int slot, int fiber, int chan)
{
  int wc = fiber * 2 - 1;
  if (chan > 127) {
    chan -= 128;
